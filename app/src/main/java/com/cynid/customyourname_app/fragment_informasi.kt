package com.cynid.customyourname_app

import android.content.Intent
import android.net.Uri
import android.os.Bundle
import android.util.Log
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import com.google.firebase.database.ktx.database
import com.google.firebase.ktx.Firebase
import kotlinx.android.synthetic.main.fragment_informasi.*

// TODO: Rename parameter arguments, choose names that match
// the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
private const val ARG_PARAM1 = "param1"
private const val ARG_PARAM2 = "param2"

/**
 * A simple [Fragment] subclass.
 * Use the [fragment_informasi.newInstance] factory method to
 * create an instance of this fragment.
 */
class fragment_informasi : Fragment() {
    // TODO: Rename and change types of parameters
    private var param1: String? = null
    private var param2: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        arguments?.let {
            param1 = it.getString(ARG_PARAM1)
            param2 = it.getString(ARG_PARAM2)
        }
    }

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_informasi, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        btnInfoBahan.setOnClickListener {
            var parent:MainActivity= activity as MainActivity
            parent.keInfoBahan()
        }

        btnfaqInformasi.setOnClickListener{
            var parent:MainActivity= activity as MainActivity
            parent.keFAQ()
        }

        btnCaraBeli.setOnClickListener{
            var parent:MainActivity= activity as MainActivity
            parent.keCaraBeli()
        }

        btnWAinfo.setOnClickListener {
            val uri: Uri = Uri.parse("smsto:085771210008")
            val i = Intent(Intent.ACTION_SENDTO, uri)
            i.setPackage("com.whatsapp")
            startActivity(i)
        }

        btnTokpedInfo.setOnClickListener {
            val tokpedIntent = Intent(
                "android.intent.action.VIEW",
                Uri.parse("https://www.tokopedia.com/customyournameid")
            )
            startActivity(tokpedIntent)
        }

        btnShopeeInfo.setOnClickListener {
            val ShopeeIntent = Intent(
                "android.intent.action.VIEW",
                Uri.parse("https://shopee.co.id/customyournameid")
            )
            startActivity(ShopeeIntent)
        }

        btnInstaInfo.setOnClickListener {
            val instaIntent = Intent(
                "android.intent.action.VIEW",
                Uri.parse("https://www.instagram.com/customyourname.id/")
            )
            startActivity(instaIntent)
        }

        //ambil info dari firebase
        val db = Firebase.database.reference
        try {
            db.child("informasi/deskripsitoko").get().addOnSuccessListener {
                tvDeskripsiToko.setText(it.value.toString())
            }
        }catch (error:java.lang.Exception){
            Log.e("error",error.toString())
        }
    }

    companion object {
        /**
         * Use this factory method to create a new instance of
         * this fragment using the provided parameters.
         *
         * @param param1 Parameter 1.
         * @param param2 Parameter 2.
         * @return A new instance of fragment fragment_informasi.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance(param1: String, param2: String) =
            fragment_informasi().apply {
                arguments = Bundle().apply {
                    putString(ARG_PARAM1, param1)
                    putString(ARG_PARAM2, param2)
                }
            }
    }
}